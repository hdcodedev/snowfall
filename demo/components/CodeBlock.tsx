...
export default function CodeBlock({ code, className = '', language = 'bash' }: CodeBlockProps) {
  ...
  const highlighted = useMemo(() => {
    const highlightBash = (source: string) => {
    ...
    return language === 'bash' ? highlightBash(code) : highlightCode(code);
  }, [code, language]);
    ...
    <div className={`relative ${className}`}>
      <div className="flex items-center justify-between px-5 py-2.5">
        ...
        <button
          ...
          onClick={handleCopy}
          className={`
            font-body text-[10px] uppercase tracking-[0.15em] transition-colors duration-200 cursor-pointer
            focus-visible:outline-1 focus-visible:outline-offset-2 focus-visible:outline-glacier
            ${copied ? 'text-glacier' : 'text-frost-dim hover:text-frost-muted'}
          `}
